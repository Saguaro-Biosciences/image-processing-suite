...
def concatenate_normalized_csv_from_s3(bucket_name, plates, base_folder_path, per_time, output_bucket, output_prefix, exp,na_cutoff, corr_3hold, local_dir="temp_data"):
    ...
    for plate in plates:
        ...
        page_iterator = paginator.paginate(Bucket=bucket_name, Prefix=f"{base_folder_path}/{plate}/")
        matching_files = [
            obj['Key']
            for page in page_iterator
            for obj in page.get('Contents', [])
            if 'Normalized_features' in obj['Key']
        ]
    ...
    if per_time:
        ...
        for timepoint in normalized_exp["Metadata_Timepoint"].unique():
            ...
            #features = df_selected.columns[~df_selected.columns.str.contains("Metadata")].tolist()
